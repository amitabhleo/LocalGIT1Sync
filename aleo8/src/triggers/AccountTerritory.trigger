...
*/
trigger AccountTerritory on Account (before insert,before update) {
    ...
    for(Account acc:Trigger.new){
        ...
        List<Territory_Members__c> termembrs =([SELECT Id, User_name__c,Territory_Name__c,
                                                ...
                                                FROM Territory_members__c where Territory_Name__r.Zip_code__c IN:billPostCodes ]);
    ...
    For(Account ac:Trigger.new){
        ...
        ashare.UserOrGroupId = terrMemberMap.get(ac.BillingPostalCode).User_name__c;
        aShares.add(ashare);
